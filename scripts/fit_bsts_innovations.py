"""Fit the innovations-form AR(2) BSTS on the full training year.

A seasonal regression with a stationary AR(2) error in innovations form and a
heteroskedastic scale. The second lag gives the error a slope at the origin
(the lag-2 structure the residual diagnostics show), which an AR(1) error
cannot carry. The likelihood has no scan, so every inference route here is
cheap and the device comparison is rerun on a level playing field.

The suite: full-rank ADVI, cold NUTS (a diagnostic) and warm-started NUTS over
the reduced-warmup grid. With ``--device cpu`` the suite refits for timing only
and writes ``{stem}.cpu.json`` sidecars next to the primary artifacts.
"""
    ...
    for kind in ("fullrank",):
        ...
        for reduced in cfg.warm_start.reduced_warmup:
            ...
            if kind == "fullrank" and reduced == reference_warmup:
                ...
                keep = max(draws["phi1"].shape[0] // cfg.vi.posterior_draws, 1)
